    ...
    async getProgressById(userId) {
      ...
      `);
        ...
        WHERE JSON_UNQUOTE(JSON_EXTRACT(data, '$.startDate')) <= CURDATE()
          AND JSON_UNQUOTE(JSON_EXTRACT(data, '$.endDate')) >= CURDATE()
      `);
            const userData = await db.sequelize.query(`SELECT fullname, email, phoneNumber FROM users WHERE userId = :userId`, {
        ...
        `, {
            ...
            const completion = examPossible > 0
            ...
            exams.forEach((exam) => {
                try {
                    ...
                    const scores = typeof exam.scores === "string"
                        ? JSON.parse(exam.scores)
                        : exam.scores;
                    if (Array.isArray(scores)) {
                        const validScores = scores.filter((s) => s && typeof s.value === "number");
                        if (validScores.length > 0) {
                            const examTotal = validScores.reduce((sum, score) => sum + score.value, 0);
            ...
            const averageQuizScore = scoreCount > 0 ? Math.round((totalScore / scoreCount) * 100) / 100 : 0;
            return {
                id: userId,
                name: user?.fullname || "",
                email: user?.email || "",
