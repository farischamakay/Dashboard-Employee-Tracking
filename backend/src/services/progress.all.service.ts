  ...
  async getProgressAll() {
    try {
      ...
      const [groups] = await db.sequelize.query(
        `SELECT groupId, title FROM groups;`
      );
      ...
      const groupProgress = groupStats.map((stat: any) => {
        const progress =
          stat.total_user > 0
      ...
      const averageQuizScore =
        validScores.length > 0
          ? parseFloat(
              (
                validScores.reduce((sum: number, val: number) => sum + val, 0) /
                validScores.length
              ).toFixed(2)
            )
          : null;
      ...
      const progressSummary = {
        ...
        users: activeUsers.map((user: any) => {
          ...
          return {
            ...
            examList: userExams.map((exam: any) => ({
              id: exam.tryoutId,
